"use client";
// LiquidEther 流体背景（reactbits.dev · backgrounds/liquid-ether）
//
// 设计：基于 Navier-Stokes 流体模拟 + Poisson 压力求解
//  - 鼠标进入：流体跟随（force = mouseForce）
//  - autoDemo：用户停止操作 N 秒后，自动在容器内游走
//  - 调色板：colors[] 用于流场浓度映射，浅色背景融合
//
// 用法（粉主题）：
...
//
// 性能：
//   - 默认 resolution=0.5（半分辨率）足以 60fps
//   - IntersectionObserver 自动暂停不可见时
//   - visibilitychange 暂停隐藏时
//   - dispose 卸载时清理 GL 上下文（防泄漏）

// 第三方组件，源码原样从 reactbits 复制，类型上有些小毛病（类字段未显式初始化、
// for 循环里 let 变量被循环外引用），不影响运行时。关闭类型检查以免误报。
/* eslint-disable */
// @ts-nocheck
...
`;
    ...
    class Viscous extends ShaderPass {
      ...
      update({ viscous, iterations, dt }: any) {
        ...
        // 提到循环外（TS let 块作用域 + 循环外 return 需要）
        let fbo_in: any = null;
        let fbo_out: any = null;
    ...
    class Output {
      simulation: any;
      // 字段在 init() 中初始化（TS strict 模式需要 ! 显式断言）
      scene!: THREE.Scene;
      camera!: THREE.Camera;
      output!: THREE.Mesh;
      ...
      addScene(mesh: THREE.Mesh) { this.scene.add(mesh); }
    ...
    colors, autoDemo, autoSpeed, autoIntensity, takeoverDuration,
    autoResumeDelay, autoRampDuration
  ]);
